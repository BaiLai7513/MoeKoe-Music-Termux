...
- 网络连接(Watt Toolkit，clash，国内代理源如清华大学代理源等）
- Ai软件(可选)
...
## 四、移动端适配（关键步骤）（此操作需要根据手机调整不建议使用，建议用步骤八-方案A或B）
...
此保活机制依旧存在冻结后台的情况可以添加入游戏助手为利用游戏白名单保活但也存在杀后台行为，极客用户直接用ROOT软件固化为系统应用保活，例如：爱玩机工具箱
## 九、架构说明
