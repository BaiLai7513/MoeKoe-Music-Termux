...
# ~/moekoe start    → 电脑浏览器模式（API: 192.168.6.228）
# ~/moekoe stop     → 停止服务
# ============================================
...
if [ "$CMD" = "stop" ]; then
    echo "→ 停止 MoeKoe Music..."
    PIDS=$(ss -tlnp 2>/dev/null | grep -E "6521|8080" | sed "s/.*pid=\([0-9]*\).*/\1/" | sort -u)
    if [ -n "$PIDS" ]; then
        for PID in $PIDS; do
            kill "$PID" 2>/dev/null || su -c "kill $PID"
        done
        echo "→ 已停止"
    else
        echo "→ 服务未在运行"
...
# 检查是否已在运行
if ss -tlnp 2>/dev/null | grep -qE "6521|8080"; then
    echo "→ MoeKoe Music 已在运行"
    exit 0
fi

# 启动 API
echo "→ 启动 MoeKoe Music..."
cd ~/MoeKoeMusic/api && node app.js --platform=lite --port=6521 &

# 电脑模式：注入热点 IP
if [ "$CMD" = "start" ]; then
    export VITE_APP_API_URL="http://192.168.6.228:6521"
    echo "→ 电脑模式: $VITE_APP_API_URL"
else
...
~/moekoe start    # 电脑浏览器模式（API 走热点 192.168.6.228）
```
...
会自动注入 `VITE_APP_API_URL=http://192.168.6.228:6521`，无需手动设置。

手动方式（备选）：

```bash
cd ~/MoeKoeMusic
VITE_APP_API_URL=http://192.168.6.228:6521 npx vite --host 0.0.0.0 --port 8080
```
...
> 项目：MoeKoe Music — 高颜值酷狗第三方播放器cd ~/MoeKoeMusic/api && node app.js --platform=lite --port=6521 &
# 自动检测热点 IP 用于跨设备访问
HOTSPOT_IP=$(ip addr show | grep "inet " | grep -v 127.0.0.1 | grep -v rmnet | grep -v meta | grep -v wlan0 | awk '{print $2}' | cut -d/ -f1 | head -1)
# 非 root 可能读不到热点接口，fallback 到 wlan0
...
> 最后更新：2026-07-24
> 设备：骁龙 8 Elite (SM8750) / ZeroTermux / Node.js v25  
> 项目：MoeKoe Music — 高颜值酷狗第三方播放器➜  Network: http://<你的IP>:8080/
```
